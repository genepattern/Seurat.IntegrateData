...
files <- unlist(strsplit(toString(args$file_list), split = " "))
data_list <- list()
...
data_list[[i]] <- read.table(file = files[[i]], header = TRUE, sep = "\t", row.names = 1)
}
...
seurat_objects[[i]] <- CreateSeuratObject(counts = data_list[[i]], project = files[[i]])
  if (length(levels(seurat_objects[[i]])) > 1){
    levels(seurat_objects[[i]]@active.ident) <- rep(files[[i]], length(levels(seurat_objects[[i]])))
  }
}
...
info(logger, message = "==========================================================")
p1 <- DimPlot(data_integrated, reduction = "umap")
